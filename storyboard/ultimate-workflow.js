const state={
    ...
    function renderFindResults(){const grid=q('findGrid'); if(!grid) return; grid.innerHTML=''; findState.results.forEach((r,i)=>{const cell=document.createElement('button'); cell.type='button'; cell.style.cssText='border:1px solid #30363d;border-radius:8px;overflow:hidden;padding:0;background:#0d1117;cursor:pointer'; const img=document.createElement('img'); img.src=r.thumb||r.full; img.alt=''; img.style.cssText='width:100%;height:120px;object-fit:cover;display:block'; cell.appendChild(img); cell.onclick=()=>pickFindResult(i); grid.appendChild(cell);});}
