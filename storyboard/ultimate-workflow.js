const state={
    ...
    async function generateImages(){
      ...
      const blockers=issues.filter(x=>x.status==='fail' && !/missing asset url/i.test(x.message));
      if(blockers.length){ q('responseBox').value=JSON.stringify({ok:false,message:'Fix validation failures before generating.',validation:blockers},null,2); addEvent('Generate blocked by validation.'); return; }
